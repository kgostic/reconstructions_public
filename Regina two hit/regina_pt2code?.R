...
get.e_ij = function(birth.year, incidence.year){
  ...
  jjs = birth.year:min(birth.year+12, incidence.year) #List of possible years of first infection (ages 0-12)
  ...
  #for loop to replace former matrix
  for(occ in 2:nn){ #2nd infection occurs by year ONE (from year 0,1,2,...) or etc
    #Negative binomial solution with r successes, x failures;
    r = 2 #Two infections in diff years
    x = occ - r #Failure all other years
    total[occ] = (gamma(x+r)/(gamma(r)*factorial(x)))* ajs[occ]^r * prod(1-ajs[1:x-1])
    #LAST TERM IS INCORRECT, fix prod(1-ajs[1:x-1])...
  } #reminder: each iteration will produce ONE probability for a specific time increment
  total
}
...
## _________________________________________
## 9. DHe1He2 - Hemagglutinin exposure 1 and exposure 2 history
## -----------------------------------------
lk.D.He1.He2 = function(pars, wmm, wmo, wom, woo, wnn, p0, xx){
  ##            ---- INPUTS ----
  ...
  # 1. Assign parameters to be fit
  He1m = pars['He1m']    #Relative susceptibility atrributable to H first exposure
  He2m = pars['He2m']    #Ditto for H second exposure
  
  # 2. calculate p_i as a function of the parameters:
  #pp = p0*(He1m*He2m*wmm + He1m*wmo + He2m*wom + woo + wnn)
  pp = p0*(He1m*He2m*wmm + He1m*(wmo) + He2m*wom + (woo + wnn))
  pp = pp/rowSums(pp)
...
## _________________________________________
## 10. DAHe1He2 - Age risk, hemagglutinin exposure 1 and exposure 2 history
## -----------------------------------------
lk.D.A.He1.He2 = function(pars, wmm, wmo, wom, woo, wnn, uc, ua, ue, p0, xx){
  ##            ---- INPUTS ----
  # wmm -- fraction of each birth cohort with matched imprinting to H e1 and e2
  # wmo -- fraction of each birth cohort with matched imprinting to H e1 and mismatched imprinting to e2
  # wom --fraction of each birth cohort with mismatched imprinting to H e1 and matched imprinting to e2
  ...
  # 1. Assign parameters to be fit
  He1m = pars['He1m']    #Relative susceptibility atrributable to H first exposure
  He2m = pars['He2m']    #Relative susceptibility of those with matched H second exposure
  Ac = pars['Ac']    #Relative susceptibility of children 0-4
  Ae = pars['Ae']    #Relative susceptibility of the elderly, 65+
  
  # 2. calculate p_i as a function of the parameters:
  pp = p0*(He1m*He2m*wmm + He1m*(wmo) + He2m*wom + (woo + wnn))*(Ac*uc+ua+Ae*ue)
...
## 10. DAHe1He2 - Age risk, hemagglutinin exposure 1 and exposure 2 history
## -----------------------------------------
lk.D.A.He1.He2.prof = function(pars, prof.par, prof.par.value, wmm, wmo, wom, woo, wnn, uc, ua, ue, p0, xx){
  
  # 1. Assign parameters to be fit
  He1m = ifelse(prof.par == 'He1m', prof.par.value, pars['He1m'])
  He2m = ifelse(prof.par == 'He2m', prof.par.value, pars['He2m'])
  Ac = ifelse(prof.par == 'Ac', prof.par.value, pars['Ac'])
  Ae = ifelse(prof.par == 'Ae', prof.par.value, pars['Ae'])

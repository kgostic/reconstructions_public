...
cocirculation = read.csv('~/Dropbox/R/Reconstructions//CocirculationData.csv', header = TRUE)
...
template = as.data.frame(read.csv('~/Dropbox/R/Reconstructions/Cocirculation_template_2017.csv', header = T))
rownames(template) = template[,1]; template = template[,-1]
colnames(template) = 2017:1901

# 2. Fill in 1996:1977 data from Thompson paper
Thompson.data = read.csv('~/Dropbox/R/Reconstructions/Thompson_data.csv', header = FALSE, skip = 1, col.names = c('Year', 'Group1', 'Group2', 'Source', 'X'), row.names = as.character(1977:2017))[,1:3]
...
Asia.fallback = read.csv('~/Dropbox/R/Reconstructions/Asia_fallback.csv', header = FALSE, skip = 1, col.names = c('Year', 'Group1', 'Group2', 'Source'), row.names = as.character(1997:2017))[,1:3]

Euro.fallback = read.csv('~/Dropbox/R/Reconstructions/Euro_fallback.csv', header = FALSE, skip = 1, col.names = c('Year', 'Group1', 'Group2', 'Source'), row.names = as.character(1997:2017))[,1:3]
